...
FROM `avana-data-warehouse-prod.ACUMATICA.vwFinanceDashboard_NetIncome_OperatingCash_EBITDA`  
WHERE FinPeriodID >= 202401
    AND FinPeriodID <= 202412
    AND AcctName='AVANA CUSO LLC'
GROUP BY ROLLUP(AcctName,AccountClassID)
ORDER BY 3 DESC,4 DESC,7 DESC
*/
CREATE VIEW `avana-data-warehouse-prod.ACUMATICA.vwFinanceDashboard_NetIncome_OperatingCash_EBITDA` AS 
